...
from slang import (
    fixed_step_chunker, SpectralProjector
)
...
def mk_featurizer(
        chk_size=2048, chk_step=None, n_features=4, log_factor: float = 2
):
    return SpectralProjector.for_sizes(chk_size, n_features=n_features,
...
def mk_wf_to_fvs(
        wf, chk_size=2048, chk_step=None, n_features=DFLT_N_FEATURES,
        log_factor: float = 2
):
...
def auto_spectral_anomaly_learner(
        wf, chk_size=2048, chk_step=None, n_features=DFLT_N_FEATURES,
        n_centroids=None, log_factor: float = 2,
        learner=OutlierModel(),
        #     learner=GmmStroll(),
        #     learner=OneClassSVM(gamma='auto'),
):
    wf_to_fvs = mk_wf_to_fvs(
        chk_size=chk_size, chk_step=chk_step, n_features=n_features,
        log_factor=log_factor
    )
...
wav_file_to_array = Pipe(
    file_to_bytes, decode_wav_bytes, itemgetter(0), np.array, np.transpose,
    itemgetter(0)
)
